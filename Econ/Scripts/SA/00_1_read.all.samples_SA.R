...
all.samples <- all.samples %>% 
  mutate(Scenario2 = case_when(
    Scenario == 1 ~ "NO_V",
    Scenario == 12 | Scenario == 4 ~ "CU_V",
    Scenario == 36 | Scenario == 28 ~ "IM_V",
    Scenario == 61 | Scenario == 53 ~ "IE_V",
    Scenario == 83 | Scenario == 75 ~ "IB_V",
    TRUE ~ "U_V"
  ),
  Scenario3 = case_when(
    Scenario == 1 ~ "No vaccination",
    Scenario == 12 | Scenario == 4 ~ "Current seasonal",
    Scenario == 36 | Scenario == 28 ~ "Improved (minimal)",
    Scenario == 61 | Scenario == 53 ~ "Improved (efficacy)",
    Scenario == 83 | Scenario == 75 ~ "Improved (breadth)",
    TRUE ~ "Universal"
  ))
