...
storage_prop <- data.table(matrix(ncol=11))# by epidemic
...
for(epidemic in 1:length(epidemics_list)){
  ...
  for(scenario in target_scenarios){
    ...
    columns_to_sum <- colnames(storage_data)[c(3:(2+num_age_groups))]
    epi <- epidemic
...
total_cases_time[,Date := ISOweek2date(week)]
